...
Singleton::Singleton() : engine(::vulkan::Engine()), seq(engine.sequence()) {
  ...
  //   cached_algorithms.try_emplace("stage_1", std::move(stage_1_shader));
  ...
  //   auto shader_name = "tmp_single_radixsort_warp" +
  ...
  //               })
  //           ->set_push_constants<RadixSortPushConstants>({
  //               .n = static_cast<uint32_t>(app_data.get_n_input()),
  //           })
  //           ->build();

  //   cached_algorithms.try_emplace("stage_2", std::move(stage_2_shader));
  ...
  //                             std::to_string(get_vulkan_warp_size()) +
  //                             ".comp";
  ...
  //   cached_algorithms.try_emplace("build_octree",
  //   std::move(build_octree_algo));
}
...
void Singleton::process_stage_1(tree::AppData &app_data_ref) {
  ...
  //   auto algo = cached_algorithms.at("stage_1").get();
  ...
  std::iota(app_data_ref.u_morton_keys_s1.begin(),
            app_data_ref.u_morton_keys_s1.end(),
            0);
  std::mt19937 rng(42);
  std::ranges::shuffle(app_data_ref.u_morton_keys_s1, rng);
}
...
void Singleton::process_stage_2(tree::AppData &app_data_ref) {
  ...
  //   auto algo = cached_algorithms.at("stage_2").get();
  ...
  static auto algo =
      engine
          ...
          ->place_holder_push_constants<Ps>()
          ->build();
