// Auto-generated code for schedule: 3A021JEHN02756_CifarDense_schedule_031
// Device ID: 3A021JEHN02756

#include "3A021JEHN02756_CifarDense_schedule_031.hpp"
...
void stage_group_chunk2(moodycamel::ConcurrentQueue<Task>& in_q, moodycamel::ConcurrentQueue<Task>& out_q) {
  while (!done.load(std::memory_order_acquire)) {
    ...
    if (in_q.try_dequeue(task)) {
      run_stages<6, 6, ProcessorType::kMediumCore, 2>(task.app_data);
      out_q.enqueue(task);
    } else {
