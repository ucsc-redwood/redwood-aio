// Auto-generated code for schedule: 3A021JEHN02756_CifarDense_schedule_019
// Device ID: 3A021JEHN02756

#include "3A021JEHN02756_CifarDense_schedule_019.hpp"
...
void stage_group_chunk1(std::vector<Task>& in_tasks, moodycamel::ConcurrentQueue<Task>& out_q) {
  for (auto& task : in_tasks) {
    run_stages<1, 5, ProcessorType::kMediumCore, 2>(task.app_data);
    out_q.enqueue(task);
  }
...
void stage_group_chunk3(moodycamel::ConcurrentQueue<Task>& in_q,
  ...
  while (!done.load(std::memory_order_acquire)) {
    Task task;
    if (in_q.try_dequeue(task)) {
      run_gpu_stages<7, 8>(task.app_data);
      out_q.enqueue(task);
    } else {
      std::this_thread::yield();
    }
