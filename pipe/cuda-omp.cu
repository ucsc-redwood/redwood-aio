    ...
    while (true) {
      ...
      if (task.is_sentinel) {
...

        {
          ...
          std::cout << "Stage A Task " << task.uid << " processing" << std::endl;
        }
  ...
  std::thread t_B([&]() {
    while (true) {
      ...
      if (q_AB.try_dequeue(task)) {
        ...
        {
          std::scoped_lock lock(printMutex);
          std::cout << "Stage B Task " << task.uid << " processing" << std::endl;
        }
...
int main(int argc, char** argv) {
  ...
  // // // 200 is good on Jetson
  // constexpr auto n_tasks = 200;
  ...
  //   cifar_dense::cuda::process_stage_5(*task.app_data);
  //   cifar_dense::cuda::process_stage_6(*task.app_data);
  //   cifar_dense::cuda::process_stage_7(*task.app_data);
  //   cifar_dense::cuda::process_stage_8(*task.app_data);
  //   cifar_dense::cuda::process_stage_9(*task.app_data);
  //   cifar_dense::cuda::device_sync();

  //   std::cout << "Task " << task.uid << " done" << std::endl;
  // }
