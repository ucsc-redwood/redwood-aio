...
class OMP_Tree : public benchmark::Fixture {
 protected:
  void SetUp(benchmark::State&) override {
    appdata = std::make_unique<tree::AppData>(std::pmr::new_delete_resource());
    temp_storage = std::make_unique<tree::omp::TempStorage>(std::thread::hardware_concurrency(),
...
#pragma omp single
  ...
  void TearDown(benchmark::State&) override { appdata.reset(); }

  std::unique_ptr<tree::AppData> appdata;
  std::unique_ptr<tree::omp::TempStorage> temp_storage;
};
...
(benchmark::State& state) {
  ...
  for (auto _ : state) {
    run_baseline_unrestricted(*appdata, *temp_storage, n_threads);
  }
}
...
static void run_stage_1_little(tree::AppData& appdata,
                               tree::omp::TempStorage& temp_storage,
