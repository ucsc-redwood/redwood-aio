...
#define CHECK_CUDA(call)                                                   \
  do {                                                                     \
    ...
    if (_status != cudaSuccess) {                                          \
      std::cerr << "Error: " << cudaGetErrorString(_status)               \
 ...
 */
__global__ void matrixMul(int *p, int *q, int *r,
                          int hp, int hq, int wp, int wq)
{
...
/**
 * MatrixMul function
 * Demonstrates usage of Unified Memory and prefetch using cudaStreamAttachMemAsync.
 */
